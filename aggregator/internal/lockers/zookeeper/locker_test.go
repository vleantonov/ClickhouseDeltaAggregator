...
func TestZookeeperLocker_LockUnlock(t *testing.T) {
	...
	const basePath = "/locks1"

	conn, _, err := zk.Connect([]string{"127.0.0.1:9182", "127.0.0.1:9181", "127.0.0.1:9183"}, time.Second*10)
	require.NoError(t, err)

	l := NewZookeeperTTLLocker(conn, basePath, time.Minute, slog.Default())

	err = l.TTLLock(context.Background(), 1)
	require.NoError(t, err)

	ok, _, err := conn.Exists(basePath + "/1")
	require.NoError(t, err)
	assert.True(t, ok)

	children, _, err := conn.Children(basePath + "/1")
	require.NoError(t, err)
...
func TestZookeeperLocker_TTLLock(t *testing.T) {
	...
	const basePath = "/locks2"

	conn, _, err := zk.Connect([]string{"127.0.0.1:9182", "127.0.0.1:9181", "127.0.0.1:9183"}, time.Second*10)
	require.NoError(t, err)

	l := NewZookeeperTTLLocker(conn, basePath, time.Second*3, slog.Default())
...
func TestZookeeperLocker_ConcurrentLock(t *testing.T) {
	...
	l1 := NewZookeeperTTLLocker(conn1, basePath, time.Hour, slog.Default())
	err = l1.TTLLock(context.Background(), 1)
	require.NoError(t, err)

	go func() {
		defer wg.Done()
		l2 := NewZookeeperTTLLocker(conn2, basePath, time.Hour, slog.Default())
	...
	_, err = conn1.Sync(basePath + "/1")
	require.NoError(t, err)
	children, _, err := conn1.Children(basePath + "/1")
	require.NoError(t, err)
	assert.Empty(t, children)
}

func TestZookeeperLocker_ConcurrentTTLLock(t *testing.T) {
	const basePath = "/locks4"
	...
	l1 := NewZookeeperTTLLocker(conn1, basePath, time.Second*2, slog.Default())
	err = l1.TTLLock(context.Background(), 1)
	require.NoError(t, err)

	go func() {
		defer wg.Done()
		l2 := NewZookeeperTTLLocker(conn2, basePath, time.Second*2, slog.Default())
		err := l2.TTLLock(context.Background(), 1)
		require.NoError(t, err)
		results[1] = time.Now()
	}()
